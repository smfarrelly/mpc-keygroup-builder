...
- [x] Close the controller-planning loop with strict result-import validation,
  ...
  warnings.
- [x] Add a redistributable six-composition software showcase: dusty, ambient,
  electro, funk, house, and weird each produce four-part MIDI, five traceable
  arrangement sections, editable packaged recipes, deterministic checksums,
  family/seed selection, and explicit deferred hardware/listening status.
...
- [x] Add a dependency-aware recipe auditor with JSON, CSV, and Markdown
  catalogs, plus safe guided starters for workstation, Drum Program, Keygroup,
  and controller-page workspaces.
- Software proof now covers six reproducible composition candidates. Promote
