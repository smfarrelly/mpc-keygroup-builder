# Documentation map

The README is the front door. These focused guides hold the detail so a new
user does not have to read the project's full history before making something.

## New to the project

1. [Getting started](getting-started.md) — install or try the browser demo.
2. [CLI reference](cli-reference.md) — discover commands and conventions.
3. [Declarative schemas](schemas.md) — integrate or validate reusable files.
4. [Troubleshooting](troubleshooting.md) — recover from common failures.
5. [Portable workflow demo](portable-demo.md) — exercise a complete workflow
   without commercial samples.
6. [Three-composition showcase](composition-showcase.md) — generate contrasting
   ideas and arrangement evidence without commercial samples.

## Build and organize instruments

- [Build a Keygroup](keygroup-building.md)
- [Keygroup variants](keygroup-variants.md)
- [Range inference](keygroup-range-inference.md)
- [Program model and semantic layouts](program-model-and-layouts.md)
- [Program Designer](program-designer.md)
- [Browser demo](browser-demo.md)
- [Catalog](catalog.md)
- [Drum ideas](drum-ideas.md)

## Create music and control hardware

- [Creative MIDI](creative-midi.md)
- [Declarative MIDI control](declarative-midi-control.md)
- [Rig profiles](rig-profiles.md)
- [Hardware workflow tools](hardware-workflow-tools.md)
- [Key 37 routing capture](key37-routing-capture.md)
- [XPJ project inspector](xpj-inspector.md)
- [Plugin parameter catalog](plugin-parameters.md)
- [Declarative plugin performance pages](plugin-mapping.md)

## Sources, compatibility, and evidence

- [Sample sources and licensing](sample-sources.md)
- [Vendor document cache](vendor-documents.md)
- [Ableton source inspector](ableton-source-inspector.md)
- [MPC ecosystem survey](mpc-ecosystem-survey.md)
- [Testing framework](testing-framework.md)
- [Declarative schemas](schemas.md)
- [MPC Key 37 field review](MPC_KEY37_FIELD_REVIEW.md)

## Direction and history

- [Product roadmap](PRODUCT_ROADMAP.md)
- [Project evolution plan](MPC_Project_Evolution_Plan.md)

The roadmaps distinguish software-complete work from deferred hardware
acceptance. A generated file passing structural checks is never silently
promoted to a listening pass.
