...
def write_outputs(prefix: Path, recipe: Path, json_text: str, midi: bytes, *, force: bool) -> tuple[Path, Path]:
    ...
    for path in paths:
        if path.is_symlink():
            raise ValueError(f"harmony output may not be a symbolic link: {path}")
        if path == source:
            raise ValueError(f"harmony output may not replace the recipe: {path}")
        if path.exists() and not path.is_file():
            raise ValueError(f"harmony output is not a regular file: {path}")
